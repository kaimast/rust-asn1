...
impl Tag {
    ...
    pub fn from_bytes(mut data: &[u8]) -> ParseResult<(Tag, &[u8])> {
        ...
        data = &data[1..];
        let mut value = u128::from(tag & 0x1f);
    ...
    pub(crate) const fn new(tag: u128, class: TagClass, constructed: bool) -> Tag {
        Tag {
            value: tag,
            constructed,
            class,
    ...
    /// supported API.
    #[doc(hidden)]
    pub const fn primitive(tag: u128) -> Tag {
        Tag::new(tag, TagClass::Universal, false)
    }

    pub(crate) const fn constructed(tag: u128) -> Tag {
...
mod tests {
    ...
    fn test_constructed() {
        for i in 0..31 {
            let tag = Tag::constructed(u128::from(i));
